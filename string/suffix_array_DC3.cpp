...
{ int i;
  ...
  for(i=0;i<n;i++) wss[wv[i]]++;  for(i=1;i<m;i++) wss[i]+=wss[i-1];
  for(i=n-1;i>=0;i--) b[--wss[wv[i]]]=a[i];
}
void dc3(int *r,int *sa,int n,int m)
{
 int i,j,*rn=r+n,*san=sa+n,ta=0,tb=(n+1)/3,tbc=0,p;
 r[n]=r[n+1]=0; for(i=0;i<n;i++) if(i%3!=0) wa[tbc++]=i;
...
void init(char *buf)
{
 int n=strlen(buf),i,j,k; memset(str,0,sizeof(str));
 for(i=0;i<n;i++) str[i]=buf[i]; dc3(str,sa,n+1,128);
 for(i=0;i<n;i++){ sa[i]=sa[i+1]; rank[sa[i]]=i; }
 for(i=j=k=0;i<n;height[rank[i++]]=k)
 if(rank[i]>0) for(k?k--:0,j=sa[rank[i]-1];i+k<n && j+k<n && str[i+k]==str[j+k];k++);
 height[0]=0;
}
