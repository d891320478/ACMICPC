...
{
 edge[cnt].v=v;	 edge[cnt].w=w;	 edge[cnt].l=l; edge[cnt].next=head[u];
 head[u]=cnt++;
 edge[cnt].v=u;	 edge[cnt].w=0;	 edge[cnt].l=l; edge[cnt].next=head[v];
 head[v]=cnt++;
}
int sap()
{
...
int main()
{
    ...
    while(lft<rht)
    {
    	...
    	{
    		if(e[i].we>mid)break;
    		addedge(e[i].ue,e[i].ve,1,e[i].we);
    		addedge(e[i].ve,e[i].ue,1,e[i].we);
    	}
    	int ans=sap();
    	if(ans>=L)rht=mid;
    	else lft=mid+1;
