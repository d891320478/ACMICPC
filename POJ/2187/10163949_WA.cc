...
void Rotate(Point *ch,int m)
{
	...
	for(i=0;i<m;i++)
	{
		while(fabs(CrossProduct(ch[i],ch[i+1],ch[p+1]))>fabs(CrossProduct(ch[i],ch[i+1],ch[p])))
			p=(p+1)%m;
		dif=fabs(CrossProduct(ch[i],ch[i+1],ch[p+1])-CrossProduct(ch[i],ch[i+1],ch[p]));
		if(dif==0.0)
		{
			t1=Dis1(ch[p].x,ch[p].y,ch[i].x,ch[i].y);
			t2=Dis1(ch[p+1].x,ch[p+1].y,ch[i+1].x,ch[i+1].y);
			ans=ans>t1?ans:t1;
			ans=ans>t2?ans:t2;
		}
		else
