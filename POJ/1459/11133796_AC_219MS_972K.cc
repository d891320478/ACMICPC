...
void push(int u,int i)
{
	...
	if(e[i].v!=s && e[i].v!=t && ef[e[i].v]==0){	vis[e[i].v]=1;		q.push(e[i].v);		}
	e[i].w-=aug;	e[i^1].w+=aug;
	ef[u]-=aug;		ef[e[i].v]+=aug;
}
void relabel(int u)
{
	int mindis=INF;
...
{
	int i;
	for(i=1;i<=n;i++)	cur[i]=head[i],dis[i]=-1,ef[i]=vis[i]=0;
	BFS();	dis[s]=n;	ef[s]=(1<<30);
	for(i=head[s];i!=-1;i=e[i].next) push(s,i);
	while(! q.empty()){	int u=q.front();	q.pop();	vis[u]=0;		discharge(u);	}
	return ef[t];
}
int main ()
{
