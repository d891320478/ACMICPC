...
#include <cstring>
using namespace std;
const long V=220,E=10000;

long n,m,h[V],vh[V],start,end;
long g[V][V],a[E],u[E];
...
struct etype
{
    ...
    etype *next,*pair;
    etype(){}
    etype(long t_,long u_,etype* next_):t(t_),u(u_),next(next_){}
    void* operator new(unsigned,void* p){return p;}
} *e[V],*eb[V],Te[E+E],*Pe=Te;

long aug(long no,long m)
{
    if(no==end)return m;
    long l=m;
    for(etype *&i=e[no];i;i=i->next)if(i->u && h[i->t]+1==h[no])
    {
        long d=aug(i->t,l<i->u?l:i->u);
