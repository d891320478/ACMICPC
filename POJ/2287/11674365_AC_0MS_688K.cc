...
int main()
{
	while (scanf("%d",&n) && n)
	{
		for(int i=1;i<=n;i++) a[i]=fastget(); sort(a+1,a+1+n,cmp);
		for(int i=1;i<=n;i++) b[i]=fastget(); sort(b+1,b+1+n,cmp);
		int ans=0;
		for(int i=1,j=1,k=n,l=n;i<=k && j<=l;)
		{
			if(a[i]>b[j]) ++i,++j,ans+=200;
			else if(a[k]>b[l]) --k,--l,ans+=200;
			else {if(a[k]<b[j]) ans-=200;--k,++j;}
		}
		printf("%d\n",ans);
	}
	return 0;
