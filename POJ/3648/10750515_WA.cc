...
void tarjan(int s)
{
	...
	if(dfn[s]==low[s])
	{
		point++;
		do{
			j=stack[tot--]; color[j]=point; ins[j]=0;
		} while (j!=s);
	}
}
bool twosat(int n)
...
void findans(int n)
{
	...
	for(int j=head[i];j!=-1;j=next[j])
		...
		{
			addedge(nhead,nnext,ne,cnt,color[i],color[e[j]]);
			into[color[i]]++;
		}
	queue<int> q;
	...
	for(int i=1;i<=point;i++)
		if(color2[i]==1)
			for(int j=shead[i];j!=-1;j=snext[j])
				color[se[j]]=1;
	bool flag=0;
	for(int i=2;i<=n;i++)
		if(color[i])
...
int main ()
{
	...
	while (scanf("%d%d",&n,&m) && (n || m))
	{
		...
		while (m--)
		{
			...
			if(a=='h' && b=='w')
			{
				addedge(head,next,e,cnt,u*2+1,v*2+1);
				addedge(head,next,e,cnt,v*2,u*2);
			}
			if(a=='h' && b=='h')
			{
				addedge(head,next,e,cnt,u*2+1,v*2);
				addedge(head,next,e,cnt,v*2+1,u*2);
			}
			if(a=='w' && b=='h')
			{
				addedge(head,next,e,cnt,u*2,v*2);
				addedge(head,next,e,cnt,v*2+1,u*2+1);
			}
			if(a=='w' && b=='w')
			{
				addedge(head,next,e,cnt,u*2,v*2+1);
				addedge(head,next,e,cnt,v*2,u*2+1);
			}
		}
		point=tot=step=0;
