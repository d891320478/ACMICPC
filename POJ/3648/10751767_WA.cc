...
int main ()
{
	...
	while (scanf("%d%d",&n,&m) && (n || m))
	{
		...
		while (m--)
		{
			...
			if(a=='h' && b=='w')
			{
				ADD(head,data,next,cnt,u*2+1,v*2+1);
				ADD(head,data,next,cnt,v*2,u*2);
			}
			if(a=='h' && b=='h')
			{
				ADD(head,data,next,cnt,u*2+1,v*2);
				ADD(head,data,next,cnt,v*2+1,u*2);
			}
			if(a=='w' && b=='h')
			{
				ADD(head,data,next,cnt,u*2,v*2);
				ADD(head,data,next,cnt,v*2+1,u*2+1);
			}
			if(a=='w' && b=='w')
			{
				ADD(head,data,next,cnt,u*2,v*2+1);
				ADD(head,data,next,cnt,v*2,u*2+1);
			}
		}
		TwoSat();
	}
