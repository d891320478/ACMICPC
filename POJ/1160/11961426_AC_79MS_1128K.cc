...
int main()
{
	...
	for(int j=i;j<=n;++j)
	for(int k=i;k<=j;++k)
		sum[i][j]+=min(ABS(a[k]-a[i]),ABS(a[k]-a[j]));
	for(int i=1;i<=n;++i)
	{
		sss+=a[i];
		dp[i][1]=ABS(sss-i*a[i]);
	}
	for(int i=2;i<=m;++i)
	for(int j=i;j<=n;++j)
	{
		...
		for(int k=i-1;k<=j-1;++k)
			dp[j][i]=min(dp[j][i],sum[k][j]+dp[k][i-1]);
	}
	/*
	for(int i=1;i<=n;++i)
	{
	...
	*/
	int ans=INF;
	sss=0;
	for(int i=m;i<=n;++i) sss+=a[i];
	for(int i=m;i<=n;++i)
	{
		ans=min(ans,dp[i][m]+ABS(sss-a[i]*(n-i+1)));
		sss-=a[i];
	}
	printf("%d\n",ans);
	return 0;
