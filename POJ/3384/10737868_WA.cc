...
inline void cut(point p1,point p2)
{
	...
	for(i=1;i<=cnt;i++)
	{
		if(xmult(p[i],p2,p1)>=0.0) q[++curcnt]=p[i];
		else
		{
			if(xmult(p[i-1],p2,p1)>=0.0) q[++curcnt]=intersect(p[i],p[i-1],p1,p2);
			if(xmult(p[i+1],p2,p1)>=0.0) q[++curcnt]=intersect(p[i],p[i+1],p1,p2);
		}
	}
...
int main ()
{
	...
	for(i=1;i<=cnt;i++)
	for(j=1;j<=cnt;j++)
		if(ma<=sqrt((p[i].x-p[j].x)*(p[i].x-p[j].x)+(p[i].y-p[j].y)*(p[i].y-p[j].y)))
		{
			ma=sqrt((p[i].x-p[j].x)*(p[i].x-p[j].x)+(p[i].y-p[j].y)*(p[i].y-p[j].y));
			o1=p[i];
			o2=p[j];
		}
	if(o1.x>o2.x)swap(o1,o2);
	if(cnt>0)printf("%.4f %.4f %.4f %.4f\n",o1.x,o1.y,o2.x,o2.y);
