	...
	while(scanf("%d",&n)!=-1)
	{
		...
		for(int j=0;j<n;j++)
			if(i!=j)
			{
				...
				if(flag==-1) addedge(head,next,e,cnt,i*2,j*2+1);
				else if(flag==1) addedge(head,next,e,cnt,i*2,j*2);
				flag=0;
				...
				if(flag==-1) addedge(head,next,e,cnt,i*2+1,j*2+1);
				else if(flag==1) addedge(head,next,e,cnt,i*2+1,j*2);
			}
		point=step=0;
