...
int main()
{
	while (scanf("%d",&n)!=-1)
	{
		for(int i=1;i<=n;i++)
		{
			...
			lis[i]=lds[i]=1;
			for(int j=1;j<i;j++)
				if(a[i]-a[j]>EPS) if(lis[j]+1>lis[i]) lis[i]=lis[j]+1;
		}
		for(int i=n-1;i>0;i--)
		for(int j=i+1;j<=n;j++)
			if(a[i]-a[j]>EPS) if(lds[j]+1>lds[i]) lds[i]=lds[j]+1;
		lis[0]=lds[0]=lis[n+1]=lds[n+1]=0;
		//for(int i=1;i<=n;i++) printf("%d ",lis[i]); printf("\n");
		//for(int i=1;i<=n;i++) printf("%d ",lds[i]); printf("\n");
		int ans=0;
		for(int i=1;i<=n;i++) lis[i]=max(lis[i],lis[i-1]);
		for(int i=n;i>=1;i--) lds[i]=max(lds[i],lds[i+1]);
		for(int i=0;i<=n;i++)
