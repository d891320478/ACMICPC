...
inline double dist(point a, point b) //两个点之间的距离
{
...
double pots(point a, point b,point d) // d 到线段AB的最短距离
{
...
double mindis(point p1[] , int v1, point p2[] , int v2)  //两个凸多边形算法标程,凸包p1,顶点数v1,凸包p2,顶点数v2
{
	...
	do 
	{
		...
		if(rot < 0)
		{
			ans = min(ans,pots(p2[n2],p2[(n2+1)%v2],p1[n1]));
			n2 = (n2+1)%v2;
			if(n2 == t2) f2 = false;
		}
		else if(rot > 0)
		{
			ans = min(ans,pots(p1[n1],p1[(n1+1)%v1],p2[n2]));
			n1 = (n1+1)%v1;
			if(n1 == t1) f1 = false;
		}
		else
		{
			ans = min(ans,pots(p1[n1],p1[(n1+1)%v1],p2[n2]));
			ans = min(ans,pots(p1[n1],p1[(n1+1)%v1],p2[(n2+1)%v2]));
			ans = min(ans,pots(p2[n2],p2[(n2+1)%v2],p1[n1]));
			ans = min(ans,pots(p2[n2],p2[(n2+1)%v2],p1[(n1+1)%v1]));
