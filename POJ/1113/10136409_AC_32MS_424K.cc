...
void Solve(int n)
{
	...
	l0=l*2.0*PI;
	for(i=1;i<=top;i++)
		l0+=sqrt(pow((double)(stack[i].x-stack[i-1].x),2.0)+pow((double)(stack[i].y-stack[i-1].y),2.0));
	l0+=sqrt(pow((double)(stack[top].x-stack[0].x),2.0)+pow((double)(stack[top].y-stack[0].y),2.0));
	printf("%d\n",(int)(l0+0.5));
}
int main ()
{
