...
void Solve(int n)
{
	...
	l0=0;
	for(i=0;i<top;i++)
		l0+=sqrt((stack[i+1].x-stack[i].x)*(stack[i+1].x-stack[i].x)+(stack[i+1].y-stack[i].y)*(stack[i+1].y-stack[i].y));
	l0+=sqrt((stack[top].x-stack[0].x)*(stack[top].x-stack[0].x)+(stack[top].y-stack[0].y)*(stack[top].y-stack[0].y));
	for(i=0;i<top-1;i++)
		l0+=(l*f(stack[i],stack[i+1],stack[i+2]));
	l0+=(l*f(stack[top-1],stack[top],stack[0]))+(long)(l*f(stack[top],stack[0],stack[1]));
	printf("%.0lf\n",l0);
}
int main ()
{
