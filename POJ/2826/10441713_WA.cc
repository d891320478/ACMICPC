...
int main ()
{
	...
	for(int tt=1;tt<=t;tt++)
	{
		...
		if((g.x>=e.x && f.x>=g.x && f.y>=g.y && mult(e,g,f)>0.0) || (g.x<=e.x && f.x<=g.x && f.y>=g.y && mult(e,f,g)>0.0))
		{
			printf("0.00\n");
			continue;
		}
		if((f.x>=e.x && g.x>=f.x && g.y>=f.y && mult(e,f,g)>0.0) || (f.x<=e.x && g.x<=f.x && g.y>=f.y && mult(e,g,f)>0.0))
		{
			printf("0.00\n");
			continue;
		}
