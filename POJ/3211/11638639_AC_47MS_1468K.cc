...
int main()
{
	while (scanf("%d%d",&m,&n) && m+n)
	{
		...
		for(int i=1;i<=n;i++) scanf("%d %s",&c[i].v,c[i].s);
		sort(c+1,c+1+n,cmp);
		int _l=1,_r,ans=0;
		while (_l<=n)
		{
			for(_r=_l;_r<n && strcmp(c[_r].s,c[_r+1].s)==0;_r++);
			int sum=0;
			for(int i=_l;i<=_r;i++) sum+=c[i].v;
			m=sum/2;
			memset(f,0,sizeof(f));
			for(int i=_l;i<=_r;i++)
			for(int j=0;j<=m;j++)
			{
				f[i&1][j]=f[(i&1)^1][j];
				if(j-c[i].v>=0) f[i&1][j]=max(f[i&1][j],f[(i&1)^1][j-c[i].v]+c[i].v);
			}
			ans+=sum-f[_r&1][m];
			_l=_r+1;
		}
