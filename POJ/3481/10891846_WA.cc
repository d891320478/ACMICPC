...
int SBTdel(int &t,int v,int idx)//删除
{
	sbt[t].sz--;
	if(v==sbt[t].key && idx==sbt[t].id || v<sbt[t].key && !sbt[t].l || v>sbt[t].key && !sbt[t].r)
	{
		if(sbt[t].l && sbt[t].r)
		{	int p=SBTdel(sbt[t].l,v,idx);		sbt[t].key=sbt[p].key;	return p;	}
		else
		{	int p=t;	t=sbt[t].l+sbt[t].r;	return p;		}
	}
	else return SBTdel(v<sbt[t].key?sbt[t].l:sbt[t].r,v,idx);
}
int maxs(int t)
{
	if(sbt[t].r==0) return t;
	else return maxs(sbt[t].r);
}
int mins(int t)
{
	if(sbt[t].l==0) return t;
	else return maxs(sbt[t].l);
}
int main()
{
	root=la=0;
	...
	while (scanf("%d",&cz) && cz)
	{
		...
		if(cz==2)
		{
			...
			else
			{
				int t=maxs(root);
				printf("%d\n",sbt[t].id);
				SBTdel(root,sbt[t].key,sbt[t].id);
			}
		}
		if(cz==3)
		{
